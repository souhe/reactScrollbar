...
describe('ScrollArea component', () => {
  ...
  it('Should have proper element style when contentStyle prop is used', () => {
    let { content } = setupComponentWithMockedSizes({
  ...
  it('Should have proper scrollbars styles', () => {
    ...
    let verticalScrollbar = scrollbars.filter(
      component => component.props.type === 'vertical'
    )[0];
    let horizontalScrollbar = scrollbars.filter(
      component => component.props.type === 'horizontal'
    )[0];

    expect(verticalScrollbar.props.containerStyle).toEqual({
      test: 'verticalContainerStyle',
    });
    expect(verticalScrollbar.props.scrollbarStyle).toEqual({
      test: 'verticalScrollbarStyle',
    });
    expect(horizontalScrollbar.props.containerStyle).toEqual({
      test: 'horizontalContainerStyle',
    });
    expect(horizontalScrollbar.props.scrollbarStyle).toEqual({
      test: 'horizontalScrollbarStyle',
    });
  });
  ...
  it('handleWheel method should scroll up on scrollRight wheel event when revertWheelAxes prop is set to true', () => {
    let { instance } = setupComponentWithMockedSizes({
      swapWheelAxes: true,
    });
  ...
  it('handleKeyDown method works properly when pressing key page down', () => {
    ...
    expect(instance.state.topPosition).toBe(
      instance.state.containerHeight - 10
    );
  });
